...
elseif find(keys == KbName('tab')) %tab
    bContinue = 1;
%     write_logfile([datestr(datetime('now')) ' Continueing ...']);
...
elseif find(keys == KbName('p')) %p
    ...
    PsychPortAudio('Stop', aud_sent{word_nr, sent_nr});
...
elseif find(keys == KbName('v')) %v
    ...
    %wite_logfile([datestr(datetime('now')) ' Start attention grabber video'])
...
elseif ~isempty(keys) && ( any(ismember( keys, numberKeysNumpad )) || any(ismember( keys, numberKeysQwerty )) ) % number keys 1 t/m 9
    ...
    disp('Audio attention grabbers are not available');
    %ind = find((keys >= 30) & (keys <= 38));
    %keynr = keys(ind(1)) - 29;
    %if (keynr <= length(aud_att))
    %    PsychPortAudio('Start', aud_att{keynr}, 1, 0, 0);
    %end
    %disp(['Playing attention grabber ' num2str(keynr)]);
